...
const GoalsTracker = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const _r3 = localStorage.getItem('goals_tracker_data_v3');
        if (_r3) { setGoals(JSON.parse(_r3)); return; }

        // Migrate from v2
        const _r2 = localStorage.getItem('goals_tracker_data_v2');
        if (_r2) {
          const v2 = JSON.parse(_r2);
    ...
    const loadDark = () => { try { const r = localStorage.getItem('goals_tracker_dark_mode'); if (r) setDarkMode(JSON.parse(r)); } catch (e) {} };
    const loadCats = () => { try { const r = localStorage.getItem('goals_tracker_custom_categories'); if (r) setCustomCategories(JSON.parse(r)); } catch (e) {} };
    const loadOrder = () => { try { const r = localStorage.getItem('goals_tracker_group_order'); if (r) setGroupOrder(JSON.parse(r)); } catch (e) {} };
    loadDark();
    loadCats();
    loadOrder();
  }, []);
  ...
  useEffect(() => {
    const save = () => { try { if (goals.length > 0) localStorage.setItem('goals_tracker_data_v3', JSON.stringify(goals)); } catch (e) {} };
    save();
  }, [goals]);

  useEffect(() => {
    const save = () => { try { localStorage.setItem('goals_tracker_dark_mode', JSON.stringify(darkMode)); } catch (e) {} };
    save();
  }, [darkMode]);

  useEffect(() => {
    const save = () => { try { localStorage.setItem('goals_tracker_custom_categories', JSON.stringify(customCategories)); } catch (e) {} };
    save();
  }, [customCategories]);

  useEffect(() => {
    const save = () => { try { if (groupOrder.length > 0) localStorage.setItem('goals_tracker_group_order', JSON.stringify(groupOrder)); } catch (e) {} };
    save();
  }, [groupOrder]);

  const handleRefresh = async () => {
    setRefreshing(true);
    try { const _r = localStorage.getItem('goals_tracker_data_v3'); if (_r) setGoals(JSON.parse(_r)); } catch (e) {}
